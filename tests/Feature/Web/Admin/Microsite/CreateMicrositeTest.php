...
test('view create microsite when user is super admin', function () {
    ...
    $response = $this->get(route("microsite.create"));

    $response->assertStatus(200);
});

test('view create microsite when user has permission', function () {
    PermissionFactory::new()->createMany(Permissions::toArray());

    $role = new Role();
    $role->name = fake()->name;
    $role->syncPermissions([Permissions::CREATE_MICROSITE->value]);
    $role->save();
