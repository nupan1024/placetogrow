...
test('view create user when user is super admin', function () {
...
test('view create user when user has permission', function () {
    PermissionFactory::new()->createMany(Permissions::toArray());

    $role = new Role();
    $role->name = fake()->name;
    $role->syncPermissions([Permissions::CREATE_USER->value]);
    $role->save();
